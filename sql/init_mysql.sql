-- ============================================================
-- 组织权限管理系统 - MySQL 初始化脚本
-- 数据库名: org_sys
-- ============================================================

-- CREATE DATABASE IF NOT EXISTS org_sys DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
-- USE org_sys;
...
CREATE TABLE IF NOT EXISTS sys_position (
    ...
    created_at    DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at    DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_sys_position_org (org_id),
    CONSTRAINT fk_sys_position_org FOREIGN KEY (org_id) REFERENCES sys_org(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS sys_user_position (
    ...
    CONSTRAINT fk_sys_user_position_user FOREIGN KEY (user_id) REFERENCES sys_user(id) ON DELETE CASCADE,
    CONSTRAINT fk_sys_user_position_pos FOREIGN KEY (position_id) REFERENCES sys_position(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS sys_position_role (
    ...
    created_at  DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY uk_sys_position_role (position_id, role_id),
    CONSTRAINT fk_sys_position_role_pos FOREIGN KEY (position_id) REFERENCES sys_position(id) ON DELETE CASCADE,
    CONSTRAINT fk_sys_position_role_role FOREIGN KEY (role_id) REFERENCES sys_role(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS sys_position_data_scope (
    ...
    created_at  DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY uk_sys_position_data_scope (position_id),
    CONSTRAINT fk_sys_position_data_scope FOREIGN KEY (position_id) REFERENCES sys_position(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS sys_data_scope_org (
    ...
    org_id      INT NOT NULL,
    UNIQUE KEY uk_sys_data_scope_org (scope_id, org_id),
    CONSTRAINT fk_sys_data_scope_org_scope FOREIGN KEY (scope_id) REFERENCES sys_position_data_scope(id) ON DELETE CASCADE,
    CONSTRAINT fk_sys_data_scope_org_org FOREIGN KEY (org_id) REFERENCES sys_org(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS sys_oper_log (
    ...
    cost_ms     INT,
    INDEX idx_sys_oper_log_user (user_id),
...
-- 根组织
INSERT IGNORE INTO sys_org (id, parent_id, org_name, org_code, sort_order, status) VALUES
(1, 0, '总公司', 'ROOT', 0, 1),
(2, 1, '研发部', 'RD', 1, 1),
(3, 1, '产品部', 'PM', 2, 1),
(4, 1, '市场部', 'MK', 3, 1),
...
-- 默认职位
INSERT IGNORE INTO sys_position (id, org_id, position_name, position_code, sort_order, status, description) VALUES
...
INSERT IGNORE INTO sys_user (id, username, password, real_name, email, phone, org_id, status) VALUES
...
-- 用户-职位关联
INSERT IGNORE INTO sys_user_position (user_id, position_id, is_primary) VALUES
(1, 1, 1),
(2, 2, 1),
(3, 3, 1),
(4, 4, 1),
...
-- 职位-角色关联
INSERT IGNORE INTO sys_position_role (position_id, role_id) VALUES
(1, 1),
(2, 3),
(3, 4),
(4, 4),
...
-- 职位数据权限配置
INSERT IGNORE INTO sys_position_data_scope (position_id, scope_type) VALUES
(1, 'all'),
(2, 'dept_and_sub'),
(3, 'self'),
(4, 'self'),
...
-- 菜单初始数据
INSERT IGNORE INTO sys_menu (id, parent_id, menu_name, menu_type, path, component, permission, icon, sort_order, visible, status) VALUES
...
-- 按钮权限
INSERT IGNORE INTO sys_menu (parent_id, menu_name, menu_type, permission, sort_order, status) VALUES
